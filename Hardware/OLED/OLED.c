...
//	#define OLED_CS   PTD10_OUT//片选已经在电路上接地，在这给软件一个不存在的引脚
...
/*
   ...
   DC  
   LPC_GPIO2->DATA|=(1<<3);//OLED_DC=1
   LPC_GPIO2->DATA&=~(1<<3);//OLED_DC=0
   
   SDA 
   LPC_GPIO2->DATA|=(1<<2);//OLED_SDA=1
   LPC_GPIO2->DATA&=~(1<<2);//OLED_SDA=0
   
   SCL
   LPC_GPIO2->DATA|=(1<<1);//OLED_SCL=1
   LPC_GPIO2->DATA&=~(1<<1);//OLED_SCL=0   
...
//void OLED_YWH_FANG(u8 x1,u8 y1,u8 x2,u8 y2)
//{
//	u8 n;
